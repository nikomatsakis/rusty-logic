...
\frac{
    \begin{gather}
    \text{impl}\langle\overline{X}\rangle\:T\:\text{for}\:\tau_i\:\text{where}\:\overline{W} \{ \} \in P \\
...
- Given an impl declaration in program $P$ that implements trait $T$ for type $\tau_i$ with type parameters $\overline{X}$ and where clauses $\overline{W}$
